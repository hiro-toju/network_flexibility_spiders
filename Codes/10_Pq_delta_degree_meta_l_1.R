...
deg_l_meta <- right_join(data.frame(Name=names(degree_meta), Degree = degree_meta), hinfo[,1:11], by="Name")
il <- data.frame(deg_l_meta, col=rep("grey70", times=nrow(deg_l_meta)))
...
dOS_degree <- left_join(dOS_max_l, il, by="Name")

g[[1]] <- ggplot(dOS_degree, aes(x=Degree, y= dOS_l_max, color=col)) + geom_point(size=3) + geom_label_repel(aes(label =Name)) + scale_colour_identity(guide="legend") + theme(legend.position="none")
...
dST_degree <- left_join(dST_max_l, il, by="Name")

g[[2]] <- ggplot(dST_degree, aes(x=Degree, y= dST_l_max, color=col)) + geom_point(size=3) + geom_label_repel(aes(label =Name)) + scale_colour_identity(guide="legend") + theme(legend.position="none")
...
dOSpWN_degree <- left_join(dOSpWN_max_l, il, by="Name")

g[[3]] <- ggplot(dOSpWN_degree, aes(x=Degree, y= dOSpWN_l_max, color=col)) + geom_point(size=3) + geom_label_repel(aes(label =Name)) + scale_colour_identity(guide="legend") + theme(legend.position="none")
...
dS_degree <- left_join(dS_max_l, il, by="Name")

g[[4]] <- ggplot(dS_degree, aes(x=Degree, y= dS_l_max, color=col)) + geom_point(size=3) + geom_label_repel(aes(label =Name)) + scale_colour_identity(guide="legend") + theme(legend.position="none")
...
dWN_degree <- left_join(dWN_max_l, il, by="Name")

g[[5]] <- ggplot(dWN_degree, aes(x=Degree, y= dWN_l_max, color=col)) + geom_point(size=3) + geom_label_repel(aes(label =Name)) + scale_colour_identity(guide="legend") + theme(legend.position="none")
...
g[[6]] <- ggplot(d3, aes(x=Degree, y=log10(dOS_dST), color=col)) + geom_point(size=3) + geom_label_repel(aes(label =Name)) + scale_colour_identity(guide="legend") + theme(legend.position="none")
...
ggsave(plot=ge, filename="../Output/delta_degree_meta_l_Pq.pdf", w=18, h=11)
...
r[1,1] <- cor.test(dOS_degree$Degree, dOS_degree$dOS_l_max, method="spearman")$estimate
r[2,1] <- cor.test(dST_degree$Degree, dST_degree$dST_l_max, method="spearman")$estimate
r[3,1] <- cor.test(dOSpWN_degree$Degree, dOSpWN_degree$dOSpWN_l_max, method="spearman")$estimate
r[4,1] <- cor.test(dS_degree$Degree, dS_degree$dS_l_max, method="spearman")$estimate
r[5,1] <- cor.test(dWN_degree$Degree, dWN_degree$dWN_l_max, method="spearman")$estimate
r[6,1] <- cor.test(d3$Degree, d3$dOS_dST, method="spearman")$estimate

r[1,2] <- cor.test(dOS_degree$Degree, dOS_degree$dOS_l_max, method="spearman")$p.value
r[2,2] <- cor.test(dST_degree$Degree, dST_degree$dST_l_max, method="spearman")$p.value
r[3,2] <- cor.test(dOSpWN_degree$Degree, dOSpWN_degree$dOSpWN_l_max, method="spearman")$p.value
r[4,2] <- cor.test(dS_degree$Degree, dS_degree$dS_l_max, method="spearman")$p.value
r[5,2] <- cor.test(dWN_degree$Degree, dWN_degree$dWN_l_max, method="spearman")$p.value
...
write.table(r2, file="../Output/correlation_delta_degree_l_Pq.txt", sep='\t', quote=F, row.names=F)
