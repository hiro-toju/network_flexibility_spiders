...
for (i in month) {
	...
	x <- full_join(dOS_u[, c("Name", i)], z[-grep('H_', z$Name),], by="Name")
	y <- data.frame(x, rep(i, times=nrow(x)))
	colnames(y)[2:4] <- c("deltaOS", "Degree", "Month")
	dOS_dp <- rbind(dOS_dp, y)
}

pdf('../Output/Degree_dOS_eachmonth_u_Fq.pdf', w= 15, h= 7)

g <- ggplot(dOS_dp, aes(x=Degree, y= deltaOS)) + geom_point(colour="dodgerblue3", size=3) + facet_wrap(. ~ factor(Month, levels=month), ncol=4) + geom_label_repel(aes(label =Name))
...
for (i in month) {
	z <- data.frame(Name=names(degree_each[[i]]), degree_each[[i]])
	x <- full_join(dST_u[, c("Name", i)], z[-grep('H_', z$Name),], by="Name")
	y <- data.frame(x, rep(i, times=nrow(x)))
	colnames(y)[2:4] <- c("deltaST", "Degree", "Month")
	dST_dp <- rbind(dST_dp, y)
}

pdf('../Output/Degree_dST_eachmonth_u_Fq.pdf', w= 15, h= 7)

g <- ggplot(dST_dp, aes(x=Degree, y= deltaST)) + geom_point(colour="dodgerblue3", size=3) + facet_wrap(. ~ factor(Month, levels=month), ncol=4) + geom_label_repel(aes(label =Name))
...
for (i in month) {
	z <- data.frame(Name=names(degree_each[[i]]), degree_each[[i]])
	x <- full_join(dOSpWN_u[, c("Name", i)], z[-grep('H_', z$Name),], by="Name")
	y <- data.frame(x, rep(i, times=nrow(x)))
	colnames(y)[2:4] <- c("deltaOSpWN", "Degree", "Month")
	dOSpWN_dp <- rbind(dOSpWN_dp, y)
}

pdf('../Output/Degree_dOSpWN_eachmonth_u_Fq.pdf', w= 15, h= 7)

g <- ggplot(dOSpWN_dp, aes(x=Degree, y= deltaOSpWN)) + geom_point(colour="dodgerblue3", size=3) + facet_wrap(. ~ factor(Month, levels=month), ncol=4) + geom_label_repel(aes(label =Name))
...
for (i in month) {
	z <- data.frame(Name=names(degree_each[[i]]), degree_each[[i]])
	x <- full_join(dS_u[, c("Name", i)], z[-grep('H_', z$Name),], by="Name")
	y <- data.frame(x, rep(i, times=nrow(x)))
	colnames(y)[2:4] <- c("deltaS", "Degree", "Month")
	dS_dp <- rbind(dS_dp, y)
}

pdf('../Output/Degree_dS_eachmonth_u_Fq.pdf', w= 15, h= 7)

g <- ggplot(dS_dp, aes(x=Degree, y= deltaS)) + geom_point(colour="dodgerblue3", size=3) + facet_wrap(. ~ factor(Month, levels=month), ncol=4) + geom_label_repel(aes(label =Name))
...
for (i in month) {
	z <- data.frame(Name=names(degree_each[[i]]), degree_each[[i]])
	x <- full_join(dWN_u[, c("Name", i)], z[-grep('H_', z$Name),], by="Name")
	y <- data.frame(x, rep(i, times=nrow(x)))
	colnames(y)[2:4] <- c("deltaWN", "Degree", "Month")
	dWN_dp <- rbind(dWN_dp, y)
}

pdf('../Output/Degree_dWN_eachmonth_u_Fq.pdf', w= 15, h= 7)

g <- ggplot(dWN_dp, aes(x=Degree, y= deltaWN)) + geom_point(colour="dodgerblue3", size=3) + facet_wrap(. ~ factor(Month, levels=month), ncol=4) + geom_label_repel(aes(label =Name))
