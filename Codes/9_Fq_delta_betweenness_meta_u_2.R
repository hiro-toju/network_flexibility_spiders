...
bet <- data.frame(Name=names(betweenness_v_meta), Betweenness=betweenness_v_meta)

dOS_bet <- left_join(dOS_max_u, bet, by="Name")

g[[1]] <- ggplot(dOS_bet, aes(x=Betweenness, y= dOS_u_max)) + geom_point(colour="dodgerblue3", size=3) + geom_label_repel(aes(label =Name))
...
dST_bet <- left_join(dST_max_u, bet, by="Name")

g[[2]] <- ggplot(dST_bet, aes(x=Betweenness, y= dST_u_max)) + geom_point(colour="chartreuse4", size=3) + geom_label_repel(aes(label =Name))
...
dOSpWN_bet <- left_join(dOSpWN_max_u, bet, by="Name")

g[[3]] <- ggplot(dOSpWN_bet, aes(x=Betweenness, y= dOSpWN_u_max)) + geom_point(colour="firebrick2", size=3) + geom_label_repel(aes(label =Name))
...
dS_bet <- left_join(dS_max_u, bet, by="Name")

g[[4]] <- ggplot(dS_bet, aes(x=Betweenness, y= dS_u_max)) + geom_point(colour="darkorchid4", size=3) + geom_label_repel(aes(label =Name))
...
dWN_bet <- left_join(dWN_max_u, bet, by="Name")

g[[5]] <- ggplot(dWN_bet, aes(x=Betweenness, y= dWN_u_max)) + geom_point(colour="burlywood4", size=3) + geom_label_repel(aes(label =Name))
...
d3 <- left_join(d2, data.frame(Name=names(betweenness_v_meta), Betweenness=betweenness_v_meta), by="Name")

g[[6]] <- ggplot(d3, aes(x= Betweenness, y=log10(dOS_dST))) + geom_point(colour="goldenrod2", size=3) + geom_label_repel(aes(label =Name))
...
ggsave(plot=ge, filename="../Output/delta_bet_meta_u_Fq.pdf", w=18, h=11)
...
r[1,1] <- cor.test(dOS_bet$Betweenness, dOS_bet$dOS_u_max, method="spearman")$estimate
r[2,1] <- cor.test(dST_bet$Betweenness, dST_bet$dST_u_max, method="spearman")$estimate
r[3,1] <- cor.test(dOSpWN_bet$Betweenness, dOSpWN_bet$dOSpWN_u_max, method="spearman")$estimate
r[4,1] <- cor.test(dS_bet$Betweenness, dS_bet$dS_u_max, method="spearman")$estimate
r[5,1] <- cor.test(dWN_bet$Betweenness, dWN_bet$dWN_u_max, method="spearman")$estimate
r[6,1] <- cor.test(d3$Betweenness, d3$dOS_dST, method="spearman")$estimate


r[1,2] <- cor.test(dOS_bet$Betweenness, dOS_bet$dOS_u_max, method="spearman")$p.value
r[2,2] <- cor.test(dST_bet$Betweenness, dST_bet$dST_u_max, method="spearman")$p.value
r[3,2] <- cor.test(dOSpWN_bet$Betweenness, dOSpWN_bet$dOSpWN_u_max, method="spearman")$p.value
r[4,2] <- cor.test(dS_bet$Betweenness, dS_bet$dS_u_max, method="spearman")$p.value
r[5,2] <- cor.test(dWN_bet$Betweenness, dWN_bet$dWN_u_max, method="spearman")$p.value
...
write.table(r2, file="../Output/correlation_delta_bet_u_Fq.txt", sep='\t', quote=F, row.names=F)
