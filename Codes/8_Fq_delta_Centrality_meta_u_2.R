...
d <- left_join(dOS_max_u, deg, by="Name")
d <- left_join(d, bet, by="Name")
d <- left_join(d, eigen, by="Name")
dOS_cent <- left_join(d, close, by="Name")
...
g[[1]] <- ggplot(dOS_cent, aes(x= degree_meta, y= dOS_u_max)) + geom_point(colour="dodgerblue3", size=3) + geom_label_repel(aes(label =Name)) + theme(legend.position="none") + xlab('Network degree')

g[[2]] <- ggplot(dOS_cent, aes(x= betweenness_meta, y= dOS_u_max)) + geom_point(colour="dodgerblue3", size=3) + geom_label_repel(aes(label =Name)) + theme(legend.position="none") + xlab('Betweenness centrality')

g[[3]] <- ggplot(dOS_cent, aes(x= eigenvectorcent_meta, y= dOS_u_max)) + geom_point(colour="dodgerblue3", size=3) + geom_label_repel(aes(label =Name)) + theme(legend.position="none") + xlab('Eigenvector centrality')

g[[4]] <- ggplot(dOS_cent, aes(x= closeness_meta, y= dOS_u_max)) + geom_point(colour="dodgerblue3", size=3) + geom_label_repel(aes(label =Name)) + theme(legend.position="none") + xlab('Closeness centrality')

##########################################################################

ge2 <- grid.arrange(g[[1]], g[[2]], g[[3]], g[[4]], nrow=4, ncol=1)

ggsave(plot=ge2, filename="../Output/delta_centrality_meta_u_Fq.pdf", w=5, h=18)
