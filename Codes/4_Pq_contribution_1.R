...
beta_meta <- data.frame(foreach(i=1:8, .combine="rbind") %dopar% betalinkr(webs2array(list(mat[[i]], meta)), partitioning="poisot", binary=FALSE, partition.st=TRUE, partition.rr=FALSE))

beta_meta <- data.frame(beta_meta, OSpWN=beta_meta$OS/beta_meta$WN)

rownames(beta_meta) <- month

saveRDS(beta_meta, file="../Output/beta_meta_Pq.rds")
write.table(cbind(month, beta_meta), file="../Output/beta_meta_Pq.txt", quote=F, sep='\t', row.names=F)

##########################################################################
# Beta-diversity: comparison with consecutive months

beta_cons <- data.frame(foreach(i=1:7, .combine="rbind") %dopar% betalinkr(webs2array(list(mat[[i]], mat[[i+1]])), partitioning="poisot", binary=FALSE, partition.st=TRUE, partition.rr=FALSE))

Transition <- c("4->5", "5->6", "6->7", "7->8", "8->9", "9->10", "10->11")
rownames(beta_cons) <- Transition

saveRDS(beta_cons, file="../Output/beta_cons_Pq.rds")
write.table(cbind(Transition, beta_cons), file="../Output/beta_cons_Pq.txt", quote=F, sep='\t', row.names=F)
...
for (j in 1:8) {
	beta_removal[[j]] <- data.frame(foreach(i=1:length(list_u), .combine="rbind") %dopar% betalinkr(webs2array(list(each_sel[[j]][[i]], meta_sel[[i]])), partitioning="poisot", binary=FALSE, partition.st=TRUE, partition.rr=FALSE))
...
saveRDS(beta_removal, file="../Output/beta_removal_u_Pq.rds")
...
for (i in 1:ncol(beta_meta)) {
	saveRDS(delta[[i]], file=sprintf("../Output/delta_u_%s_Pq.rds", colnames(beta_meta)[i]))
}
...
for (j in 1:8) {
	beta_removal[[j]] <- data.frame(foreach(i=1:length(list_l), .combine="rbind") %dopar% betalinkr(webs2array(list(each_sel[[j]][[i]], meta_sel[[i]])), partitioning="poisot", binary=FALSE, partition.st=TRUE, partition.rr=FALSE))
...
saveRDS(beta_removal, file="../Output/beta_removal_l_Pq.rds")
...
for (i in 1:ncol(beta_meta)) {
	saveRDS(delta[[i]], file=sprintf("../Output/delta_l_%s_Pq.rds", colnames(beta_meta)[i]))
}
...
saveRDS(presence_u, file="../Output/presence_u_Pq.rds")
saveRDS(presence_l, file="../Output/presence_l_Pq.rds")
