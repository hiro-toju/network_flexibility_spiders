...
for (i in month) {
	z <- data.frame(Name=names(degree_each[[i]]), degree_each[[i]])
	x <- full_join(dOS_l[, c("Name", i)], z[grep('H_', z$Name),], by="Name")
	x2 <- left_join(x, hinfo[,1:11], by="Name")
	x3 <- data.frame(x2, col=rep("grey70", times=nrow(x2)))
	y <- data.frame(x3, rep(i, times=nrow(x3)))
	colnames(y)[c(2,3,15)] <- c("deltaOS", "Degree", "Month")
...
pdf('../Output/Degree_dOS_eachmonth_l_Fq.pdf', w= 15, h= 7)

g <- ggplot(dOS_dp, aes(x= Degree, y= deltaOS, color=col)) + geom_point(size=3) + facet_wrap(. ~ factor(Month, levels=month), ncol=4) + geom_label_repel(aes(label =Name)) + scale_colour_identity(guide="legend") + theme(legend.position="none")
...
for (i in month) {
	z <- data.frame(Name=names(degree_each[[i]]), degree_each[[i]])
	x <- full_join(dST_l[, c("Name", i)], z[grep('H_', z$Name),], by="Name")
	x2 <- left_join(x, hinfo[,1:11], by="Name")
	x3 <- data.frame(x2, col=rep("grey70", times=nrow(x2)))
	y <- data.frame(x3, rep(i, times=nrow(x3)))
	colnames(y)[c(2,3,15)] <- c("deltaST", "Degree", "Month")
...
pdf('../Output/Degree_dST_eachmonth_l_Fq.pdf', w= 15, h= 7)

g <- ggplot(dST_dp, aes(x= Degree, y= deltaST, color=col)) + geom_point(size=3) + facet_wrap(. ~ factor(Month, levels=month), ncol=4) + geom_label_repel(aes(label =Name)) + scale_colour_identity(guide="legend") + theme(legend.position="none")
...
for (i in month) {
	z <- data.frame(Name=names(degree_each[[i]]), degree_each[[i]])
	x <- full_join(dOSpWN_l[, c("Name", i)], z[grep('H_', z$Name),], by="Name")
	x2 <- left_join(x, hinfo[,1:11], by="Name")
	x3 <- data.frame(x2, col=rep("grey70", times=nrow(x2)))
	y <- data.frame(x3, rep(i, times=nrow(x3)))
	colnames(y)[c(2,3,15)] <- c("deltaOSpWN", "Degree", "Month")
...
pdf('../Output/Degree_dOSpWN_eachmonth_l_Fq.pdf', w= 15, h= 7)

g <- ggplot(dOSpWN_dp, aes(x= Degree, y= deltaOSpWN, color=col)) + geom_point(size=3) + facet_wrap(. ~ factor(Month, levels=month), ncol=4) + geom_label_repel(aes(label =Name)) + scale_colour_identity(guide="legend") + theme(legend.position="none")
...
for (i in month) {
	z <- data.frame(Name=names(degree_each[[i]]), degree_each[[i]])
	x <- full_join(dS_l[, c("Name", i)], z[grep('H_', z$Name),], by="Name")
	x2 <- left_join(x, hinfo[,1:11], by="Name")
	x3 <- data.frame(x2, col=rep("grey70", times=nrow(x2)))
	y <- data.frame(x3, rep(i, times=nrow(x3)))
	colnames(y)[c(2,3,15)] <- c("deltaS", "Degree", "Month")
	dS_dp <- rbind(dS_dp, y)
}
...
pdf('../Output/Degree_dS_eachmonth_l_Fq.pdf', w= 15, h= 7)

g <- ggplot(dS_dp, aes(x= Degree, y= deltaS, color=col)) + geom_point(size=3) + facet_wrap(. ~ factor(Month, levels=month), ncol=4) + geom_label_repel(aes(label =Name)) + scale_colour_identity(guide="legend") + theme(legend.position="none")
...
for (i in month) {
	z <- data.frame(Name=names(degree_each[[i]]), degree_each[[i]])
	x <- full_join(dWN_l[, c("Name", i)], z[grep('H_', z$Name),], by="Name")
	x2 <- left_join(x, hinfo[,1:11], by="Name")
	x3 <- data.frame(x2, col=rep("grey70", times=nrow(x2)))
	y <- data.frame(x3, rep(i, times=nrow(x3)))
	colnames(y)[c(2,3,15)] <- c("deltaWN", "Degree", "Month")
...
pdf('../Output/Degree_dWN_eachmonth_l_Fq.pdf', w= 15, h= 7)

g <- ggplot(dWN_dp, aes(x= Degree, y= deltaWN, color=col)) + geom_point(size=3) + facet_wrap(. ~ factor(Month, levels=month), ncol=4) + geom_label_repel(aes(label =Name)) + scale_colour_identity(guide="legend") + theme(legend.position="none")
