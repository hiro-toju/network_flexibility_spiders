...
dOS_dp <- full_join(dOS_max_u, dprime_u_meta, by="Name")

g[[1]] <- ggplot(dOS_dp, aes(x=1-dprime_meta, y= dOS_u_max)) + geom_point(colour="dodgerblue3", size=3) + geom_label_repel(aes(label =Name))
...
g[[2]] <- ggplot(dST_dp, aes(x=1-dprime_meta, y= dST_u_max)) + geom_point(colour="chartreuse4", size=3) + geom_label_repel(aes(label =Name))
...
dOSpWN_dp <- full_join(dOSpWN_max_u, dprime_u_meta, by="Name")

g[[3]] <- ggplot(dOSpWN_dp, aes(x=1-dprime_meta, y= dOSpWN_u_max)) + geom_point(colour="firebrick2", size=3) + geom_label_repel(aes(label =Name))
...
dS_dp <- full_join(dS_max_u, dprime_u_meta, by="Name")

g[[4]] <- ggplot(dS_dp, aes(x=1-dprime_meta, y= dS_u_max)) + geom_point(colour="darkorchid4", size=3) + geom_label_repel(aes(label =Name))
...
dWN_dp <- full_join(dWN_max_u, dprime_u_meta, by="Name")

g[[5]] <- ggplot(dWN_dp, aes(x=1-dprime_meta, y= dWN_u_max)) + geom_point(colour="burlywood4", size=3) + geom_label_repel(aes(label =Name))

##########################################################################
# delta OS/delta ST vs. interaction generality: metaweb
...
g[[6]] <- ggplot(d3, aes(x=1-dprime_meta, y=log10(dOS_dST))) + geom_point(colour="goldenrod2", size=3) + geom_label_repel(aes(label =Name))

##########################################################################

ge <- grid.arrange(g[[2]], g[[5]], g[[4]], g[[1]], g[[3]], g[[6]], nrow=2, ncol=3)

ggsave(plot=ge, filename="../Output/delta_generality_meta_u_Fq.pdf", w=18, h=11)
...
r[1,1] <- cor.test(1-dOS_dp$dprime_meta, dOS_dp$dOS_u_max, method="spearman")$estimate
r[2,1] <- cor.test(1-dST_dp$dprime_meta, dST_dp$dST_u_max, method="spearman")$estimate
r[3,1] <- cor.test(1-dOSpWN_dp$dprime_meta, dOSpWN_dp$dOSpWN_u_max, method="spearman")$estimate
r[4,1] <- cor.test(1-dS_dp$dprime_meta, dS_dp$dS_u_max, method="spearman")$estimate
r[5,1] <- cor.test(1-dWN_dp$dprime_meta, dWN_dp$dWN_u_max, method="spearman")$estimate
r[6,1] <- cor.test(1-d3$dprime_meta, d3$dOS_dST, method="spearman")$estimate

r[1,2] <- cor.test(1-dOS_dp$dprime_meta, dOS_dp$dOS_u_max, method="spearman")$p.value
r[2,2] <- cor.test(1-dST_dp$dprime_meta, dST_dp$dST_u_max, method="spearman")$p.value
r[3,2] <- cor.test(1-dOSpWN_dp$dprime_meta, dOSpWN_dp$dOSpWN_u_max, method="spearman")$p.value
r[4,2] <- cor.test(1-dS_dp$dprime_meta, dS_dp$dS_u_max, method="spearman")$p.value
r[5,2] <- cor.test(1-dWN_dp$dprime_meta, dWN_dp$dWN_u_max, method="spearman")$p.value
...
write.table(r2, file="../Output/correlation_delta_generality_u_Fq.txt", sep='\t', quote=F, row.names=F)
